...
elif [[ "${MACTAHOE_SOURCE[@]}" =~ "lib-core.sh" ]]; then
...
if [[ -d "${MACTAHOE_TMP_DIR}" ]]; then
  start_animation; sleep 2; stop_animation; echo

  if [[ -d "${MACTAHOE_TMP_DIR}" ]]; then
    prompt -e "ERROR: MacTahoe installer or tweaks is already running. Probably it's run by '$(ls -ld "${MACTAHOE_TMP_DIR}" | awk '{print $3}')'"
  fi
fi

rm -rf "${MACTAHOE_TMP_DIR}"
mkdir -p "${MACTAHOE_TMP_DIR}"; exec 2> "${MACTAHOE_TMP_DIR}/error_log.txt"
...
signal_error() {
  ...
  IFS=$'\n'
  local sources=($(basename -a "${MACTAHOE_SOURCE[@]}" "${BASH_SOURCE[@]}" | sort -u))
  local dist_ids=($(awk -F '=' '/ID/{print $2}' "/etc/os-release" | tr -d '"' | sort -Vru))
  local repo_ver=""
  local lines=()
  local log="$(awk '{printf "\033[1;31m  >>> %s\n", $0}' "${MACTAHOE_TMP_DIR}/error_log.txt" || echo "")"
  ...
  for i in "${sources[@]}"; do
    lines=($(grep -Fn "${MACTAHOE_COMMAND:-${BASH_COMMAND}}" "${REPO_DIR}/${i}" | cut -d : -f 1 || echo ""))
    prompt -e "  >>> ${i}$(IFS=';'; [[ "${lines[*]}" ]] && echo " at ${lines[*]}")"
  done

  prompt -e "SNIPPET:\n    >>> ${MACTAHOE_COMMAND:-${BASH_COMMAND}}"
  prompt -e "TRACE  :"
  ...
  if [[ "$(grep -ril "Release" "${MACTAHOE_TMP_DIR}/error_log.txt")" == "${MACTAHOE_TMP_DIR}/error_log.txt" ]]; then
